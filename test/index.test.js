...
const gw2Test = new PAPIBuilder("https://api.guildwars2.com/v2").path(
  "items",
  (items) => items.endpoint($GET, [0]).query({ lang: "", ids: [0] }, (path) => path.endpoint($GET, [itemResp]).error({ text: "" })).slug((slug) => slug.endpoint($GET, itemResp).error({ text: "" }).query({ lang: "" }))
).build();
